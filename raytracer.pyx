...
def run():
    ...
    t = time.time()
    pixels = render_test(width, height)
    print(" %f sec" % (time.time() - t))

    buf = bytearray()
    for row in pixels:
        for pixel in row:
            # buf.append((int(pixel.x) << 16) + (int(pixel.y) << 8) + int(pixel.z))
            buf.append(min(int(pixel.x), 255))
            buf.append(min(int(pixel.y), 255))
            buf.append(min(int(pixel.z), 255))
    buf2 = bytes(buf)
    im = Image.frombuffer("RGB", (width, height), buf2, 'raw', "RGB", 0, 1)
    im.save("image.png")
